...
export const getMainPage = async (page: number, sort: onlineSortType, search: string) => {
	const query =
		search != '' ? search : atob(`JTJCbWlsZiUyMCUyQmZ1bGwlMjBjb2xvciUyMCUyQmVuZ2xpc2g=`);
	const url = `${atob(API_URL)}/api/v2/search?query=${query}&sort=${sort}&page=${page}`;
	console.log('GET', url);
	return await fetchData<NHMainPageType>(url, {
		result: [],
		num_pages: 0,
		per_page: 0,
		total: 0
